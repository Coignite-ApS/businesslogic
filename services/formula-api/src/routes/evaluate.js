...
// Formula token auth preHandler
const formulaAuth = async (req, reply) => {
  const token = req.headers['x-auth-token'];
  if (!token) return reply.code(401).send({ error: 'Missing X-Auth-Token header' });
  const result = await validateFormulaToken(token);
  if (!result) return reply.code(403).send({ error: 'Invalid auth token' });
...
export async function registerRoutes(app) {
  ...
  app.post('/execute', { schema: singleSchema, preHandler: formulaAuth }, async (req, reply) => {
    ...
    const stat = (opts) => stats.record({ calculatorId: null, responseTimeMs: Date.now() - start, type: 'formula', account, ...opts });
