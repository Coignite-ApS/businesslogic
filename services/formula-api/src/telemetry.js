/**
 * OpenTelemetry instrumentation for bl-formula-api.
 *
...
export function initTelemetry() {
  ...
  const instanceId = process.env.INSTANCE_ID || `formula-${Math.random().toString(36).slice(2, 8)}`;

  const resource = new Resource({
    [ATTR_SERVICE_NAME]: 'bl-formula-api',
    [ATTR_SERVICE_VERSION]: '1.23.0',
