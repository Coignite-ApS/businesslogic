# 07. Embeddable Calculator Widget (Render Library)
...
1. **Design time** (Directus): Admin creates/edits component definitions in `widget_components`. The layout builder (#08) reads this collection to populate its component palette.
...
- Directus drag-and-drop layout builder (that's project #08)
...
### 7b: Full Component Set (ship second)
...
- Programmatic API (`Calculator` class with events)
- `prop_schema` field on `widget_components` (needed for #08 builder)
- **Scope: ~1500-2000 lines, ~2 weeks**

## Authentication Note

The widget uses **per-calculator `X-Auth-Token`** (existing) for execute/describe calls. This is separate from account-level API keys (#06) which are for MCP. Two different auth paths for two different consumers:
