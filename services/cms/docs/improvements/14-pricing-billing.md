...
### Phase D: Knowledge & AI Metering (ships with #12/#13)

13. **KB storage tracking** — sum `kb_chunks` + `kb_documents` file sizes per account
14. **AI query counter** — increment on each `/calc/kb/ask` call, check against limit
15. **KB count enforcement** — check on knowledge base creation
16. **Usage dashboard** — extend account page with all resource meters

### Phase E: Lifetime Deal Support

17. **LTD flag on account** — `account.lifetime_deal = true`, `account.ltd_tier` field
18. **Custom limits for LTD** — stored directly on account, override plan limits
19. **LTD redemption endpoint** — `POST /register` variant that accepts LTD code
20. **LTD code management** — admin UI to generate/track/expire codes
...
- **#12 (Knowledge Base)** — for KB storage and count metering (Phase D)
- **#13 (Knowledge Retrieval)** — for AI query metering (Phase D)
- **#06 (Account-Level MCP)** — for MCP access gating
- **#10 (Lead Capture)** — for lead capture level gating
...
- Phase B (new tiers): ~100 lines code + Stripe config
- Phase C (add-ons): ~300-400 lines (purchase flow, resolver, UI)
- Phase D (knowledge metering): ~150 lines (part of #12/#13 implementation)
