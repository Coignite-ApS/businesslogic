# 08. Widget Layout Builder (Directus Module)

**Status:** planned
**Phase:** 3 — Embeddable Widgets
**Replaces:** old #2 (Display Components) — builder portion
**Depends on:** #07 (Render Library must exist first)
...
- After #07, layout configs are JSON stored in `calculator_layouts`
...
### Live Preview
- Toggle button shows the actual rendered widget (using the render library from #07)
...
- [ ] Saved layout config is valid JSON matching the schema from #07
...
- **#07 (Render Library)** must be complete — the preview pane embeds it
- `calculator_layouts` collection (created in #07)
- `widget_components` collection (created in #07) — populates the component palette
- `widget_themes` collection (created in #07) — populates theme selector
- `widget_templates` collection (created in #07) — populates template selector
