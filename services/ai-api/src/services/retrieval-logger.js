...
export function logRetrievalQuality(data) {
  const {
    ...
    rerankerUsed, contextualRetrievalUsed, parentDocUsed,
    rerankerLatencyMs, featuresActive,
  } = data;
  ...
  query(
    `INSERT INTO ai_retrieval_quality
      (account_id, knowledge_base_id, conversation_id, query_text, query_type,
       ...
       reranker_used, contextual_retrieval_used, parent_doc_used,
       reranker_latency_ms, features_active)
    ...
    [
      ...
      rerankerUsed ?? false, contextualRetrievalUsed ?? false, parentDocUsed ?? false,
      rerankerLatencyMs ?? null, featuresActive ? JSON.stringify(featuresActive) : null,
    ],
  ).catch(err => {
