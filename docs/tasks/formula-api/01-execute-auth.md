...
- [x] Add auth middleware to execute routes in `src/routes/evaluate.js`
- [x] Token validation via `safeTokenCompare()` (reuse `src/utils/auth.js`)
- [x] Token cache: fetch formula tokens from Directus, cache with TTL
- [x] Update `src/services/stats.js` to include `account` + `type` in stats batch
- [x] Update calculator execute to also post `account` in stats
- [x] Strip `/health` internals unless admin token provided
...
- `src/routes/evaluate.js` — execute endpoints with auth + rate limiting
- `src/utils/auth.js` — `safeTokenCompare()`, `validateFormulaToken()`
- `src/services/account-limits.js` — shared account limit loading (extracted from calculators)
- `src/services/rate-limiter.js` — RPS + monthly rate limiting
- `src/routes/calculators.js` — calculator routes (uses same shared modules)
- `src/services/stats.js` — stats buffering + posting (includes account/type)
