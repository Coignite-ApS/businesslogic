-- Migration 030: PL/pgSQL function public.aggregate_usage_events()
-- Rolls up unaggregated usage_events into monthly_aggregates.
-- Returns JSONB: { events_aggregated, accounts_touched, periods_touched, lag_seconds }
-- Run inside an implicit transaction (PL/pgSQL function body).
...
BEGIN
  ...
  WITH new_events AS (
    SELECT
      ...
      COUNT(DISTINCT (metadata->>'calculator_id')) FILTER (WHERE event_kind = 'calc.call') AS calc_unique_calculators,
  ...
  upserted AS (
    INSERT INTO public.monthly_aggregates (
      ...
      calc_calls, calc_unique_calculators,
    ...
    ON CONFLICT (account_id, period_yyyymm) DO UPDATE SET
      ...
      calc_unique_calculators = monthly_aggregates.calc_unique_calculators + EXCLUDED.calc_unique_calculators,
  ...
  -- Step 2: mark source rows as aggregated (in same transaction)
  marked AS (
