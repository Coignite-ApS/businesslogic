-- Migration 033: aggregate_usage_events hardening — I2 + I3 + function signature change
--
-- I2: Replace 5-second date_updated heuristic with CTE-RETURNING stats (accounts_touched,
--     periods_touched are now exact counts from the upserted CTE, not a time-window query).
--
-- I3: Add p_batch_size parameter; source rows are selected with ORDER BY + LIMIT before
--     GROUP BY so memory is bounded per invocation. Cron loops until drained.
--
-- Signature change: zero-arg aggregate_usage_events() → aggregate_usage_events(int DEFAULT 100000).
-- Postgres requires DROP before CREATE when the argument list changes.
