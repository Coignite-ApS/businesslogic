-- Migration 036: aggregate_usage_events — add touched_accounts array to JSONB result
--
-- Adds a `touched_accounts` field (uuid[]) to the return JSONB so the cron handler
-- can publish per-account cache invalidation messages instead of a global ALL flush.
--
-- Source: array_agg(DISTINCT account_id) FROM the existing `upserted` CTE (migration 033).
-- All other behaviour (batch cap, advisory lock, idempotency, I1–I5 regressions) preserved.
