...
export class BlDropdown extends LitElement {
  static styles = [
    resetStyles,
    css`
      ...
      select:focus { border-color: var(--bl-primary, #3b82f6); }
    `,
  ];
  ...
  private _onChange(e: Event) {
    ...
    const opt = this.options[select.selectedIndex];
    const value = opt ? opt.const : select.value;
  ...
  render() {
    return html`
      ...
      <select id=${this.field} @change=${this._onChange}>
        ${this.options.map(
          (opt) => html`<option ?selected=${opt.const === this.value}>${opt.title}</option>`,
        )}
      </select>
    `;
  }
